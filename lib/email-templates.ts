...
export interface EmailCaseContext {
  ...
  /** ملخص الدعوى ونص المأمورية من التحليل الأولي (الموديول 1) — تُستخدم
   * لصياغة إشارة دقيقة ومحدَّدة إلى طبيعة النزاع/المأمورية في متن الخطاب
   * بدل صياغة عامة، دون اختراع أي تفصيل غير معطى فعلياً هنا. */
...
- إن أُعطيت لك ملخص الدعوى و/أو نص المأمورية، استخدمهما فعلياً لصياغة جملة افتتاحية دقيقة ومحدَّدة تربط طلب المستندات بطبيعة النزاع أو مهمة الخبرة تحديداً (مثال: بدل عبارة عامة مثل "في إطار استكمال إجراءات تدقيق ملف الدعوى"، اكتب ما يشبه "في إطار أعمال الخبرة الحسابية المكلَّف بها الخبير لفحص وتدقيق حسابات الشراكة بين الطرفين" إن كان هذا هو موضوع المأمورية فعلاً) — بحيث يبدو الخطاب مصمَّماً لهذه الدعوى تحديداً لا خطاباً معمَّماً. لا تُدرج تفاصيل من الملخص/المأمورية غير ذات صلة بطلب المستندات.
...
- لا تخترع أي تفصيل غير معطى لك صراحة في البيانات أدناه (مثل أسماء أشخاص أو تواريخ أو وقائع لم تُذكر) — الاستعانة بملخص الدعوى والمأمورية تعني صياغة أدق لما هو معطى فعلاً، لا إضافة معلومات جديدة.
...
${caseCtx.court ? `- المحكمة: ${caseCtx.court}\n` : ""}${
  ...
  }${caseCtx.caseSummary ? `- ملخص الدعوى: ${caseCtx.caseSummary}\n` : ""}${
    caseCtx.mandateText ? `- نص المأمورية: ${caseCtx.mandateText}\n` : ""
  }
